...
- Routing по SQLite (seed-файл `admin-routing.yaml` используется только при первом импорте).
- Список источников по-прежнему в `products.yaml`.
- Интерактивная админка для подписок уже доступна через Telegram-команды (`/subscribe`, `/unsubscribe`).
...
- `products.yaml` остаётся источником данных по источникам релизов.
- `admin-routing.yaml` используется как seed для инициализации routing-таблиц в SQLite.
