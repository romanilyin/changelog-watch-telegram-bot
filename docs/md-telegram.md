...
MarkdownV2 в Telegram очень капризный: нужно экранировать `_ * [ ] ( ) ~ ` > # + - = | { } . !` почти везде.  
...
## Что уже включено в текущий бот

В этом проекте очистка markdown-артефактов применяется **для каждого поста**, т.е. и для «полных».

Пайплайн сейчас такой:

1. Из релиз-данных берётся `entry.body`.
2. `compact_markdown_for_telegram()` делает нормализацию markdown:
   - удаляет комментарии и служебные маркеры
   - превращает `# / ## / ###` в читаемые заголовки
   - сводит списки к `•`/`◦`
   - убирает markdown-ссылки и inline-выделения
   - оставляет кодовые блоки как текст без обратных кавычек
   - убирает табличные разделители
3. После этого `format_message()` укорачивает текст до `max_body_chars` и делает `html.escape`.

Результат: в `sendMessage(parse_mode="HTML")` уже не прилетают необработанные `#`, `*`, `` ` `` и похожие символы.
